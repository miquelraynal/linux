						...
						struct ieee802154_channel *chan)
{
	...
	/* Preamble code [1; 23] are mandatory and rely on a 4/16MHz mean PRF */
	if (chan->preamble_code <= 23 &&
	...
	/* Channels 4, 7, 11, 15 support codes [7; 13] and [17; 20] */
	switch (chan->channel) {
	case 4:
	case 7:
	case 11:
	case 15:
		if ((chan->preamble_code >= 7 && chan->preamble_code <= 13) ||
	...
	if (chan->channel >= 9 && chan->channel <= 12)
		return true;
