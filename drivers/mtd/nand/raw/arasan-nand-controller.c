...
#define ECC_REG				0x34
#define   ECC_REG_COL(x)		FIELD_PREP(GENMASK(15, 0), (x))
#define   ECC_REG_LEN(x)		FIELD_PREP(GENMASK(26, 16), (x))
...
#define ANFC_XLNX_SDR_LS_CORE_CLK	100000000
...
static int anfc_select_target(struct nand_chip *chip, int target)
{
	...
	writel_relaxed(arasan_nand->timing_mode, nfc->base + DATA_INTERFACE_REG);
	writel_relaxed(arasan_nand->ecc_conf, nfc->base + ECC_REG);
				     ...
				     const struct nand_data_interface *conf)
{
	...
	arasan_nand->clk = ANFC_XLNX_SDR_HS_CORE_CLK;
	...
	if (of_device_is_compatible(np, "xlnx,zynqmp-nand-controller") &&
		...
		arasan_nand->clk = ANFC_XLNX_SDR_LS_CORE_CLK;
...
static irqreturn_t anfc_irq_handler(int irq, void *dev_id)
{
	...
	writel_relaxed(st, nfc->base + INTR_STS_REG);
	complete(&nfc->event);
