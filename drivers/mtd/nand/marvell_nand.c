...
#define	CHIP_DELAY_TIMEOUT (msecs_to_jiffies(200)*10) /* Arbitrary value from old driver pxa3xx_nand.c */ //todo
#define DATA_BUFFER_SIZE 4320 /*final buffer size based on printk in old driver //todo */
...
struct marvell_nfc {
	...
	u8 data_buf[DATA_BUFFER_SIZE]; //todo: how to force 32b alignment ?
	int data_pos;
...
static void marvell_nfc_trigger_read_op(struct mtd_info *mtd)
{
	...
	} while ((readl(nfc->regs + NDCR) & NDCR_ND_RUN) != 0);
...
//	int nsels;
	...
	//todo : + nsels * sizeof(struct marvell_nand_chip) ???? (check struct)
	if (!chip) {
		dev_err(dev, "could not allocate chip\n");
		return -ENOMEM;
	}

//todo	chip->nsels = 1//nsels;
	chip->selected = -1;
