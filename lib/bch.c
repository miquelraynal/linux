		...
		unsigned int len, uint8_t *ecc)
{
	...
	while (mlen--) {
		...
		p0 = tab0 + (l+1)*swap_bits(bch, w);
		p1 = tab1 + (l+1)*swap_bits(bch, w >>  8);
		p2 = tab2 + (l+1)*swap_bits(bch, w >> 16);
		p3 = tab3 + (l+1)*swap_bits(bch, w >> 24);
	       ...
	       const unsigned int *syn, unsigned int *errloc)
{
	...
	err = bch_compute_error_locator_polynomial(bch, data, len, recv_ecc,
						   calc_ecc, syn);
	if (err > 0) {
